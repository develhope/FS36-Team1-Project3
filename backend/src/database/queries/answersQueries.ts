...
export const fetchAnswersByQuestionId = async (questionId: number) => {
    const answers = await db.many("SELECT answers FROM answers WHERE questions_id = $1", [questionId]);
    return answers;
}

export const fetchCorrectAnswerByQuestionId = async (questionId: number) => {
    const correctAnswer = await db.many("SELECT answers FROM answers WHERE questions_id = $1 AND isCorrect = true", [questionId]);
