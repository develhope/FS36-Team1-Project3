...
export const insertQuestionsIntoDb = async () => {
  const questions = [
    {
      questions:
        "Quale tag HTML si usa per creare un collegamento ipertestuale?",
      arguments: "html",
    },
    {
      questions:
        "Qual è il tag corretto per creare una lista ordinata in HTML?",
      arguments: "html",
    },
    {
    ...
    {
      questions:
        "Quale metodo degli array in JavaScript restituisce il primo elemento che soddisfa una certa condizione, oppure undefined se nessun elemento soddisfa la condizione?",
      arguments: "javascript",
    },
    {
      questions:
        "Qual è l'oggetto JavaScript utilizzato per gestire operazioni asincrone?",
      arguments: "javascript",
    },
    {
      questions:
        "Come si chiama il metodo React per gestire uno stato interno in un componente funzionale?",
      arguments: "react",
    },
    {
    ...
    {
      questions:
        "Cosa succede se assegni un numero a una variabile dichiarata come string in TypeScript?",
      arguments: "typescript",
    },
    {
...
export const fetchQuestionsByArgument = async (arg: string) => {
  const questions = await db.many(
    "SELECT * FROM questions WHERE arguments = $1",
    [arg]
  );
  return questions;
};

export const fetchQuestionIdbyArgument = async (
  arg: string
): Promise<{ id: number }[]> => {
  const questionId = await db.many(
    "SELECT id FROM questions WHERE arguments = $1",
    [arg]
  );
  return questionId;
};
